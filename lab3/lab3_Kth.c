...
int main(int argc, char *argv[])
{
    ...
    FILE *resultOfTest = fopen("Results/Results_Kth.txt", "a");
...
void sort(double *M2, size_t size) {
    ...
    size_t chunkSize = size / k;
    #pragma omp parallel for num_threads(k) shared(M2, size)
    for (int i = 0; i < k; i++) {
        gnomeSort(M2 + i * chunkSize, chunkSize);
    }
