...
int main(int argc, char *argv[])
{
    ...
    FILE *resultOfTest = fopen("Results/Results_with_auto_shedule.txt", "a");
    double min = MAXNUM;
...
#pragma omp parallel for default(none) reduction(min:min) private(j) shared(M2,N) schedule(auto) num_threads(threadCount)
        for (j = 0; j < M2SIZE; j++)
        {
            if (M2[j] < min)
            {
...
#pragma omp parallel for default(none) reduction(+:sum) private(j) shared(M2, min, N) schedule(auto) num_threads(threadCount)
        for (j = 0; j < M2SIZE; j++)
        {
            if ((int)(M2[j] / min) % 2 == 0)
            {
