...
#pragma omp parallel for default(none) private(j) shared(M1, N, chunkSize) schedule(static, chunkSize) num_threads(threadCount)
        for (j = 0; j < M1SIZE; j++)
        {
...
#pragma omp parallel for default(none) private(j) shared(M2, M2temp, N, chunkSize) schedule(static, chunkSize) num_threads(threadCount)
        for (j = 0; j < M2SIZE; j++)
        {
            M2temp[j] = M2[j];
        }
...
// Merge
#pragma omp parallel for default(none) private(j) shared(M2, M1, N, chunkSize) schedule(static, chunkSize) num_threads(threadCount)
        for (j = 0; j < M2SIZE; j++)
        {
            M2[j] = M1[j] / M2[j];
        }
